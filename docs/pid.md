...
3. **RLS helper functions** live in the `auth` schema and are the only thing policies call. At a minimum: `auth.current_profile_id()`, `auth.current_role()`, `auth.is_care_circle_member(circle_id uuid)`, `auth.is_company_member(company_id uuid)`, `auth.is_provider_verified()`, `auth.can_see_care_plan(care_plan_id uuid)`. Every policy is a one-liner calling one of these helpers. Policies that inline SQL joins are rejected in review.
...
**C2. Auth, role management & shared audit log (W2 anchor)**
